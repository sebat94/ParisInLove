$(document).ready(function(){
    ...
    $('#contactForm').submit(function(){
      ...
      if($contactName.val().length == 0){
        $contactName.css("border-color","rgb(173, 52, 30)");
        $contactName.prev().css("color", "rgb(173, 52, 30)");
      }
      if($contactEmail.val().length == 0){
        $contactEmail.css("border-color","rgb(173, 52, 30)");
        $contactEmail.prev().css("color", "rgb(173, 52, 30)");
      }
      if($contactTitle.val().length == 0){
        $contactTitle.css("border-color","rgb(173, 52, 30)");
        $contactTitle.prev().css("color", "rgb(173, 52, 30)");
      }
      if($contactDescription.val().length == 0){
        $contactDescription.css("border-color","rgb(173, 52, 30)");
        $contactDescription.prev().css("color", "rgb(173, 52, 30)");
      }
